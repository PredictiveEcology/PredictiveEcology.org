...
out <- setupProject(
  ...
  paths = list(projectPath = "~/SpaDES_book/prepInputs")
)
```
...
```{r}
SK <- SpaDES.project::setupStudyArea(
  list(level = 2, NAME_1 = "Saskatchewan", NAME_2 = "Division No. 9$")
)
...
```{r}
SK <- SpaDES.project::setupStudyArea(
  list(level = 2, NAME_1 = "Saskatchewan",
       NAME_2 = "Division No. 9$", epsg = 5070)
)
...
```{r custom}
LuxPath <- "~/SpaDES_book/prepInputs"
LuxembourgFull <- prepInputs(
  destinationPath = LuxPath,
  # preProcess keeps file from this:
  dlFun = geodata::gadm(name = "GADM", country = "LUX",
                        level = 0, path = LuxPath)
) |> Cache()
...
terra::plot(LuxembourgFull, add = TRUE)
```
...
fileName <- file.path("R/Chapter_scripts",
                      sub("rmarkdown", "R", knitr::current_input()))
```
