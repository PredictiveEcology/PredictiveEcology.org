...
The first thing in `RSFpredict.R` is a `defineModule()` block. It
contains nothing executable yet -- it's a *declaration* of what the
module is. Three pieces matter:
...
## Step 2: the `init` event

`doEvent.RSFpredict()` is the function `SpaDES` calls every time one of
this module's events fires. It's a `switch()` on the event name. The
**`init` event** runs exactly once, at the start of the simulation.
This is where the linear script's *setup section* lives -- but with one
extra job: telling `SpaDES` what to do next.
...
## Step 3: the recurring events

`simLayers` and `simRSFmap` are the body of the original annual loop,
split at the natural seam: build inputs, then predict from them. Each
ends by scheduling **itself** at `time(sim) + predictionInterval`, so
they recur:
