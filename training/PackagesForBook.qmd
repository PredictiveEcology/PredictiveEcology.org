...
out <- setupProject(
  ...
  packages = c(
    "PredictiveEcology/Require@usePak",
    ...
    "PredictiveEcology/PredictiveEcology.org@training-book/training/R/pkgList.R"
  ),
  options = list(repos = repos),
  paths = list(projectPath = "~/SpaDES_book/Introduction"),
  modules = "PredictiveEcology/Biomass_core@main"
)
```

Note that `setupProject` will change your working directory to
`~/SpaDES_book/Introduction`. If that is not where you want to keep
working, restart R and make sure you set your working directory to
where you want it to be (if restarting is not sufficient).
