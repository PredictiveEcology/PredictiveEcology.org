...
#| cache: false
#| purl: false
out1 <- SpaDES.project::setupProject(
  packages = c("googledrive", "httr2"),
  paths = list(projectPath = "~/SpaDES_book/LandRDemo_coreVeg"),
  options = list(repos = repos),
  verbose = -2)
if (!googledrive::drive_has_token() && !interactive()) {
  # Use gargle's default per-OS cache (~/.cache/gargle on Linux,
  # ~/Library/Caches/gargle on macOS, %LOCALAPPDATA%/gargle/Cache on
  # Windows). Setting only the email lets gargle pick the matching
  # cached token without an explicit (platform-specific) cache path.
  options(gargle_oauth_email =
            Sys.getenv("GARGLE_OAUTH_EMAIL", "eliotmcintire@gmail.com"))
}
```
...
out <- setupProject(
  ...
  packages = c("PredictiveEcology/Require@usePak",
               ...
               "DiagrammeR", "googledrive", "httr2", "terra"),
...
out2$paths$outputPath <- reproducible::normPath(
  file.path("~/SpaDES_book/LandRDemo_coreVeg", "outputsSim2"))
